...
class _BaseSettings:
    _cli_opts = {'RPA'              : ('rpa', None),
                 ...
                 'Include'          : ('include', []),
                 'Exclude'          : ('exclude', []),
                 'OutputDir'        : ('outputdir', abspath('Reports/out_')),
                 'Log'              : ('log', 'log.html'),
                 'Report'           : ('report', 'report.html'),
                 'XUnit'            : ('xunit', None),
                 'SplitLog'         : ('splitlog', False),
                 'TimestampOutputs' : ('timestampoutputs', True),
                 'LogTitle'         : ('logtitle', None),
                 'ReportTitle'      : ('reporttitle', 'PMR Automation Report'),
    ...
    def _get_output_file(self, option):
        ...
        if self['TimestampOutputs']:
            path = abspath(os.path.join(self['OutputDir']+self.start_timestamp, name))
        else:
...
class RobotSettings(_BaseSettings):
    _extra_cli_opts = {'Extension'          : ('extension', ('.robot', '.rbt', '.robot.rst')),
                       ...
                       'Listeners'          : ('listener', ['allure_robotframework']),
